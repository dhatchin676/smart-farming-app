// fix_railway.js
// Run this from: C:\Users\dhatc\OneDrive\Documents\GitHub\smart-farming-app
// Command: node fix_railway.js
// It fixes all Railway deployment issues directly

const fs   = require('fs');
const path = require('path');

const DB_STUB = `// config/db.js — MySQL stub (not used, MongoDB only)
const pool = {
  query:   async () => [[], []],
  execute: async () => [[], []],
};
...
const files = [
  { path: path.join(base, 'backend', 'config', 'db.js'),  content: DB_STUB },
  { path: path.join(base, 'backend', 'server.js'),         content: SERVER  },
];

files.forEach(({path: fp, content}) => {
...
console.log('  git add backend/server.js backend/config/db.js');
console.log('  git commit -m "fix: stub db.js + fix server.js for Railway"');
